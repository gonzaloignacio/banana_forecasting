...
# Get time series
banana_act_ts <- ts(banana$Actual, start = c(2024, 1), freq = 52)
...
arima_mod <- arima(train_ts, order = c(4, 0, 2))
...
write.csv2(final_model, "banana_exp_predictions.csv", row.names = FALSE)
write.csv2(results, "best_models_accuracy_exp.csv", row.names = FALSE)
...
write.csv2(all_models_results, "all_models_results_exp.csv", row.names = FALSE)

arima_full <- arima(banana_act_ts, order = c(4, 0, 2))
...
write.csv2(predictions_2025, "predictions_2025_exp.csv", row.names = FALSE)
